...
from utils import *
from models import TrainLogReg, TrainSVM
...
start = time. time()
higgs = sqlContext.read.load('gs://bigdata_data/Datasets/HIGGS/HIGGS.csv', 
...
# Load and parse the data file, converting it to a DataFrame.
start = time. time()
hepmass = sqlContext.read.load('gs://bigdata_data/Datasets/HEPMASS/all_train.csv', 
                          format='com.databricks.spark.csv', 
                          header='true', 
                          delimiter=',',
                          inferSchema='true')
